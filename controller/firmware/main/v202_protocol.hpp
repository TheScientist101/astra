...
static uint8_t freq_hopping[][16] = {
    {0x27, 0x1B, 0x39, 0x28, 0x24, 0x22, 0x2E, 0x36, 0x19, 0x21, 0x29, 0x14,
     0x1E, 0x12, 0x2D, 0x18}, //  00
    {0x2E, 0x33, 0x25, 0x38, 0x19, 0x12, 0x18, 0x16, 0x2A, 0x1C, 0x1F, 0x37,
     0x2F, 0x23, 0x34, 0x10}, //  01
    {0x11, 0x1A, 0x35, 0x24, 0x28, 0x18, 0x25, 0x2A, 0x32, 0x2C, 0x14, 0x27,
     0x36, 0x34, 0x1C, 0x17}, //  02
    {0x22, 0x27, 0x17, 0x39, 0x34, 0x28, 0x2B, 0x1D, 0x18, 0x2A, 0x21, 0x38,
...
enum txType { AETR = 0, TAER };

enum rxState { NO_BIND = 0, WAIT_FIRST_SYNCHRO, BOUND, SIGNAL_LOST };
...
enum rxReturn {
  ...
  BIND_IN_PROGRESS,  // Bind in progress, first frame has been received with TX
                     // id, wait no bind frame
...
public:
  ...
  void init(NRF24 *wireless, uint8_t mode, uint8_t txType);

  void setTXId(uint8_t txid[3]);
  void command(uint8_t throttle, int8_t yaw, int8_t pitch, int8_t roll,
               uint8_t flags);
...
protected:
  ...
  bool isBindingFrame();
  uint8_t getCRC();
  inline bool checkTXaddr() {
    return (mTxid[0] == mFrame[7] && mTxid[1] == mFrame[8] &&
            mTxid[2] == mFrame[9]);
  }
